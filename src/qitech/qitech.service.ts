...
export class QitechService {
  ...
  private CLIENT_PRIVATE_KEY = fs.readFileSync("jwtECDSASHA512.key");
  ...
  getAccountOwner(escrow) {
    return {
      ...
      birth_date: dateFormat(
        escrow.escrowAccountOwnerList[0].birth_date,
        "yyyy-MM-dd"
      ),
      document_identification:
        escrow.escrowAccountOwnerList[0].document_identification_attach,
      email: escrow.escrowAccountOwnerList[0].email,
      individual_document_number:
        escrow.escrowAccountOwnerList[0].individual_document_number,
      ...
      proof_of_residence:
        escrow.escrowAccountOwnerList[0].proof_of_residence_attach,
    };
  }

  getAccountManagerList(escrow) {
    let company_representatives = [];
    escrow.escrowAccountManagerList[0].escrowAccountManagerRepresentativeList.forEach(
      (e) => {
    ...
    return {
      account_manager: {
        ...
        company_document_number:
          escrow.escrowAccountManagerList[0].company_document_number,
        company_representatives: company_representatives,
        company_statute:
          escrow.escrowAccountManagerList[0].company_statute_attach,
        directors_election_minute:
          escrow.escrowAccountManagerList[0].directors_election_minute,
        email: escrow.escrowAccountManagerList[0].email,
        foundation_date: dateFormat(
          escrow.escrowAccountManagerList[0].foundation_date,
          "yyyy-MM-dd"
        ),
  ...
  async findByStatus(status: string) {
    return await this.escrowRepository
  ...
  generateEncodedBody = (body) => {
    return jwt.sign({ ...body }, this.CLIENT_PRIVATE_KEY, {
      algorithm: "ES512",
      noTimestamp: true,
    });
  };
    ...
    const request = {
      ...
      header: {
        ...
        Authorization:
          ...
          this.API_CLIENT_KEY +
          ":" +
          this.generateEncodedBody(bodyJWT),
      },
      endpoint: "/upload",
      isFormData: true,
    };
